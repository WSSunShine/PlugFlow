...
NSString *const QMTVGPUImageBeautyFragmentShaderString = SHADER_STRING
(
...
@implementation QMTV_GPUImageBeautyFilter

- (id)init;
{
    if (!(self = [super initWithFragmentShaderFromString:QMTVGPUImageBeautyFragmentShaderString]))
    {
		return nil;
    }
...
- (void)setBeautyWithToneLevel:(CGFloat)toneLevel beautyLevel:(CGFloat)beautyLevel brightLevel :(CGFloat)brightLevel{
    if (beautyLevel != _beautyLevel || toneLevel !=_toneLevel) {
        [self setParams:beautyLevel tone:toneLevel];
    }
    if (brightLevel != _brightLevel) {
        [self setBrightLevel:brightLevel];
    }
}
